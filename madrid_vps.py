...
class CustomPDF(FPDF):
    ...
    # 🔒 Blindaje Unicode compatible con cualquier firma de FPDF
...
# Fin Clase personalizada para el PDF con encabezado y pie de página
...
def generar_pdf(datos, x, y, filename):
    ...
    pdf.cell(0, 10, "INFORME TÉCNICO ORIENTATIVO DE AFECCIONES AL MEDIO NATURAL", ln=True, align="C")
    pdf.set_font("Arial", "B", 16)
    pdf.set_text_color(0, 0, 0)
    pdf.cell(0, 7, "(Documento auxiliar de apoyo a la tramitación administrativa)", ln=True, align="C")
    pdf.ln(10)
    ...
    seccion_titulo("1. Datos aportados para la elaboración del informe")
...
def main():
    ...
    # 🔐 ONE-SHOT: identificar usuario SOLO al entrar
    st.session_state.setdefault("user_id", None)

    if st.session_state["user_id"] is None:
        params = st.query_params
        if "uid" not in params:
            st.error("Acceso no válido. Vuelve al panel.")
            st.stop()
        try:
            st.session_state["user_id"] = int(params["uid"])
        except ValueError:
            st.error("UID inválido")
            st.stop()

    # --- A partir de aquí, YA hay user_id fijo ---
    ...
    if st.session_state.get("mapa_html") and st.session_state.get("pdf_file"):
        ...
        if st.download_button(
            ...
            if st.session_state.get("user_id") is None:
                st.error("Sesión no válida. Vuelve al panel.")
                st.stop()
    ...
    if st.session_state.get("registro_enviado"):
        st.success("Informe registrado correctamente. Volviendo al panel…")
        st.markdown(
